...
func _enter_tree():
	print("   → BlueGuy _enter_tree: Authority is ", get_multiplayer_authority())
...
func _ready():
	...
	print("   → BlueGuy _ready: name=", name, " | my_peer_id=", my_id, " | node_authority=", my_authority, " | is_authority=", is_auth)
	...
	print("   → BlueGuy _ready: Animation set to idle")
	
	# Only enable camera for the player we control
	var camera = get_node_or_null("Camera2D")
	if camera:
		camera.enabled = is_auth
		print("   → BlueGuy _ready: Camera enabled=", camera.enabled, " for is_authority=", is_auth)
	else:
		print("   → BlueGuy _ready: Camera NOT FOUND!")
	...
	print("   → BlueGuy _ready: Scene root is: ", root.name if root else "NULL")
...
func request_player_name():
	...
	if network_manager and network_manager.has("player_names"):
		...
		if network_manager.player_names.has(player_id):
			...
			print("   → BlueGuy requested name: ", player_name)
